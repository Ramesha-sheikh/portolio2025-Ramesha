...
const BlogHeader: React.FC = () => {
  ...
  useEffect(() => {
    if (themeBtnRef.current) {
      themeBtnRef.current.ariaLabel = theme ?? "light";
    }
  }, [theme]);
      ...
      >
        <div className="w-full h-full mx-auto max-w-6xl flex items-center justify-between">
          ...
          <nav className="flex items-center">
            <div className="glassmorphism md:bg-transparent">
              <ul className="flex justify-evenly items-center py-1">
                {navLinks.map((navLink) => (
                  ...
                  >
                    <Link
                      href={navLink.url}
                      className={`text-sm md:text-lg flex flex-col items-center w-[4.5rem] md:w-auto md:mr-6 link-outline
                        ${
